...
//  be in its hierarchy. If the creation time of the child is not later than
//  the creation time of the parent, it is assumed that parent "inherited"
//  the child and did not actually start it. Such "inherited" children are
/// not killed.
//
//  Parameters:
//	  dwProcessId - identifier of the process to terminate
...
DWORD WINAPI KillProcessTreeWinHelper(DWORD dwProcessId)
{
	...
	while (current)
	{
		...
		do
		{
			...
			// (or equal, just to be safe), it is not a candidate for being killed
			...
			if (hChild &&
				...
				CompareFileTime(&creationTime, &processCreated) > 0)
			{
